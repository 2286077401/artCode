<template>
	<view class="tn-slider-class tn-slider" :class="{'tn-slider--disabled': disabled}" :style="{
      backgroundColor: inactiveColor
    }" @tap="click">
		<!-- slider滑动线 -->
		<view class="tn-slider__gap" :style="[
        barStyle,
        {
          ...
          backgroundColor: activeColor
			...
			<!-- slider滑块 -->
			<view class="tn-slider__button-wrap" @touchstart="touchStart" @touchmove="touchMove" @touchend="touchEnd"
				@touchcancel="touchEnd">
				<view v-if="$slots.default || $slots.$default">
					<slot></slot>
				</view>
				<view v-else class="tn-slider__button" :style="[blockStyle, {
	...
	export default {
		...
		methods: {
			...
			click(event) {
				...
				const value = (((event.detail.x - this.sliderRect.left) / this.sliderRect.width) * (this.max - this.min)) +
					this.min
