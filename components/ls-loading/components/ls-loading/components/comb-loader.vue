<template>
	<view class="text-container">
		<!-- #ifdef APP-NVUE -->
		<text ref="text" class="loading-text" :class="animationClass" v-for="(item, index) in texts" :key="index" :style="{'font-size': fontSize + 'upx'}">{{item}}</text>
		<!-- #endif -->
		<!-- #ifdef APP-VUE || H5 -->
		<text ref="text" class="loading-text" :class="animationClass" v-for="(item, index) in texts" :key="index" :style="{'animation-delay': 102.631578 * (index + 1) + 'ms', 'font-size': fontSize + 'upx'}" v-html="item"></text>
		<!-- #endif -->
		<!-- #ifdef MP-WEIXIN -->
		<block v-for="(item, index) in texts" :key="index">
			<text v-if="item == '&nbsp;'" :style="{width: spaceSize + 'upx'}"></text>
			<text ref="text" class="loading-text" :class="animationClass" :style="{'animation-delay': 102.631578 * (index + 1) + 'ms', 'font-size': fontSize + 'upx'}" v-else>{{item}}</text>
		</block>
		<!-- #endif -->
	</view>
</template>
